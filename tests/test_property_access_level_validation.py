    ...
    **Validates: Requirements 12.3, 12.7, 12.8**
    ...
    def test_property_28_read_only_allows_select(self, table_name):
        ...
        **Validates: Requirements 12.3, 12.7, 12.8**
        ...
        result = AccessLevelValidator.validate_query(query, AccessLevel.READ_ONLY, [])
    ...
    def test_property_28_read_only_rejects_write_operations(self, table_name, operation):
        ...
        **Validates: Requirements 12.3, 12.7, 12.8**
        ...
        assert is_err(result), f"{operation} query should be rejected with READ_ONLY access"

        error_msg = unwrap_err(result)
        assert "read-only" in error_msg.lower(), f"Error message should mention 'read-only', got: {error_msg}"
        assert operation in error_msg, f"Error message should mention operation '{operation}', got: {error_msg}"
    ...
    def test_property_28_read_only_rejects_ddl_operations(self, table_name, ddl_operation):
        ...
        **Validates: Requirements 12.3, 12.7, 12.8**
        ...
        assert is_err(result), f"{ddl_operation} query should be rejected with READ_ONLY access"

        error_msg = unwrap_err(result)
        assert "read-only" in error_msg.lower(), f"Error message should mention 'read-only', got: {error_msg}"
        assert ddl_operation in error_msg, f"Error message should mention operation '{ddl_operation}', got: {error_msg}"
    ...
    **Validates: Requirements 12.4, 12.7, 12.8**
    ...
    def test_property_29_read_write_allows_dml_operations(self, table_name, operation):
        ...
        **Validates: Requirements 12.4, 12.7, 12.8**
        ...
        result = AccessLevelValidator.validate_query(query, AccessLevel.READ_WRITE, [])
    ...
    def test_property_29_read_write_rejects_ddl_operations(self, table_name, ddl_operation):
        ...
        **Validates: Requirements 12.4, 12.7, 12.8**
        ...
        # DDL operations should be rejected
        assert is_err(result), f"{ddl_operation} query should be rejected with READ_WRITE access"

        error_msg = unwrap_err(result)
        assert "read-write" in error_msg.lower(), f"Error message should mention 'read-write', got: {error_msg}"
        assert ddl_operation in error_msg, f"Error message should mention operation '{ddl_operation}', got: {error_msg}"
        assert "DDL" in error_msg, f"Error message should mention 'DDL', got: {error_msg}"
    ...
    **Validates: Requirements 12.5, 12.7, 12.8**
    ...
    def test_property_30_table_specific_read_allows_select_on_allowed_tables(self, allowed_tables):
        ...
        **Validates: Requirements 12.5, 12.7, 12.8**
        ...
        result = AccessLevelValidator.validate_query(query, AccessLevel.TABLE_SPECIFIC_READ, allowed_tables)
    ...
    @given(
        allowed_tables=st.lists(st.sampled_from(["users", "orders", "products"]), min_size=1, max_size=2, unique=True),
        disallowed_table=st.sampled_from(["customers", "invoices", "payments", "shipments"]),
    )
    @settings(max_examples=100)
    def test_property_30_table_specific_read_rejects_select_on_disallowed_tables(
        self, allowed_tables, disallowed_table
    ):
        """
        Property 30: Access Level Query Validation - Table Specific Read
    ...
    @given(
        allowed_tables=st.lists(st.sampled_from(["users", "orders", "products"]), min_size=1, max_size=3, unique=True),
        operation=st.sampled_from(["INSERT", "UPDATE", "DELETE"]),
    )
    @settings(max_examples=100)
    ...
    @given(
        allowed_tables=st.lists(st.sampled_from(["users", "orders", "products"]), min_size=2, max_size=3, unique=True)
    )
    @settings(max_examples=100)
    ...
    @given(
        allowed_tables=st.lists(st.sampled_from(["users", "orders"]), min_size=1, max_size=2, unique=True),
    ...
    **Validates: Requirements 12.6, 12.7, 12.8**
    ...
    def test_property_31_table_specific_read_write_allows_all_dml_on_allowed_tables(self, allowed_tables, operation):
        ...
        **Validates: Requirements 12.6, 12.7, 12.8**
        ...
        result = AccessLevelValidator.validate_query(query, AccessLevel.TABLE_SPECIFIC_READ_WRITE, allowed_tables)
    ...
    @settings(max_examples=100)
    def test_property_31_table_specific_read_write_rejects_operations_on_disallowed_tables(
        self, allowed_tables, disallowed_table, operation
    ):
        """
        Property 31: Access Level Query Validation - Table Specific Read Write
    ...
    @settings(max_examples=100)
    def test_property_31_table_specific_read_write_allows_operations_with_joins_on_allowed_tables(
        self, allowed_tables, operation
    ):
        """
        Property 31: Access Level Query Validation - Table Specific Read Write
...
class TestAccessLevelValidationEdgeCases:
    ...
    def test_empty_query_rejected_for_all_access_levels(self, access_level):
        ...
        result = AccessLevelValidator.validate_query("", access_level, ["users", "orders"])

        assert is_err(result), f"Empty query should be rejected for {access_level.value} access"

        error_msg = unwrap_err(result)
        assert "empty" in error_msg.lower(), f"Error message should mention 'empty', got: {error_msg}"

    @given(access_level=st.sampled_from([AccessLevel.TABLE_SPECIFIC_READ, AccessLevel.TABLE_SPECIFIC_READ_WRITE]))
    @settings(max_examples=50)
    def test_table_specific_access_requires_allowed_tables(self, access_level):
        ...
        assert is_err(result), f"Query should be rejected when allowed_tables is empty for {access_level.value} access"

        error_msg = unwrap_err(result)
        assert "allowed_tables" in error_msg, f"Error message should mention 'allowed_tables', got: {error_msg}"

    @given(
        query_case=st.sampled_from(["lower", "upper", "mixed"]),
        access_level=st.sampled_from([AccessLevel.READ_ONLY, AccessLevel.READ_WRITE]),
    )
    @settings(max_examples=50)
    def test_case_insensitive_query_parsing(self, query_case, access_level):
        ...
        result = AccessLevelValidator.validate_query(query, access_level, [])

...
class TestPersistenceProperties:
    ...
    def test_source_citation_completeness(self, citation):
        ...
        **Validates: Requirements 6.3, 6.4**
        ...
        assert citation.source_type in ["web", "database"], \
            "Source citation must have a valid source_type"
        
        # Verify that the citation has an identifier
        assert citation.identifier is not None, \
            "Source citation must have an identifier"
        assert len(citation.identifier) > 0, \
            "Source citation identifier must not be empty"
        ...
        if citation.source_type == "database":
            ...
            assert isinstance(citation.identifier, str), \
                "Database citation identifier must be a string (table name)"
            assert len(citation.identifier) > 0, \
                "Database citation must include a non-empty table name"
            ...
            assert "[Database]" in formatted, \
                "Database citation display must indicate source type"
        
        # Requirement 6.4: Web citations must include URL
        elif citation.source_type == "web":
            # The identifier should be a valid URL
            assert isinstance(citation.identifier, str), \
                "Web citation identifier must be a string (URL)"
            assert len(citation.identifier) > 0, \
                "Web citation must include a non-empty URL"
            ...
            formatted = citation.format_for_display()
            assert citation.identifier in formatted, \
                f"Web citation display must include URL '{citation.identifier}'"
            assert "[Web]" in formatted, \
                "Web citation display must indicate source type"
        
        # Verify that relevance score is present and valid
        assert citation.relevance_score is not None, \
            "Source citation must have a relevance score"
        ...
        assert citation.identifier in formatted_with_relevance, \
            "Citation with relevance must include the identifier"
    ...
    def test_conversation_history_display_includes_sources(self, role, content, sources):
        ...
        **Validates: Requirements 11.2**
        ...
        assert message.sources is not None, \
            "RAG-enhanced message must have sources"
        assert len(message.sources) > 0, \
            "RAG-enhanced message must have at least one source"
        ...
        assert formatted is not None, \
            "Message format_for_display must return a value"
        assert len(formatted) > 0, \
            "Formatted message must not be empty"
        
        # The formatted display must include the message content
        assert content in formatted, \
            f"Formatted display must include message content"
        ...
        if has_web_sources:
            assert has_web_indicator, \
                "Formatted display must include [Web] indicator for web sources"
        
        if has_db_sources:
            assert has_db_indicator, \
                "Formatted display must include [Database] indicator for database sources"
        ...
        has_sources_section = any(
            ...
            for keyword in ["source", "citation", "reference"]
        )
        assert has_sources_section, \
            "Formatted display must have a section indicating sources were used"
    ...
    def test_non_rag_message_display_without_sources(self, role, content):
        ...
        assert message.sources is None, \
            "Non-RAG message should have sources=None"
        ...
        assert formatted is not None, \
            "Non-RAG message format_for_display must return a value"
        assert len(formatted) > 0, \
            "Formatted non-RAG message must not be empty"
        
        # The formatted display must include the message content
        assert content in formatted, \
            "Formatted display must include message content"

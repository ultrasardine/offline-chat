...
def invalid_url_strategy(draw):
    ...
    if invalid_type == "empty":
        ...
        protocol = draw(st.sampled_from(["http", "https"]))
        invalid_text = draw(
            st.text(alphabet=st.characters(whitelist_categories=("Zs", "Cc", "Cs")), min_size=1, max_size=10)
        )
...
def valid_url_strategy(draw):
    ...
    domain_parts = draw(
        st.lists(
            st.text(alphabet="abcdefghijklmnopqrstuvwxyz0123456789-", min_size=1, max_size=15).filter(
                lambda x: x and x[0] != "-" and x[-1] != "-" and x != "-"
            ),
            min_size=2,
            max_size=4,
        )
...
class TestWebScraperProperties:
    ...
    def test_url_validation_rejects_invalid_formats(self, invalid_url):
        ...
        **Validates: Requirements 1.3**
        ...
        assert not is_valid, f"Expected invalid URL to be rejected, but was accepted: {repr(invalid_url)}"
        ...
        assert not is_valid_ks, (
            f"Expected invalid URL to be rejected by knowledge source validator, but was accepted: {repr(invalid_url)}"
        )

        # Should provide an error message
        assert error_message is not None, f"Expected error message for invalid URL, but got None: {repr(invalid_url)}"
        assert isinstance(error_message, str), f"Expected error message to be a string, got {type(error_message)}"
        assert len(error_message) > 0, f"Expected non-empty error message for invalid URL: {repr(invalid_url)}"
    ...
    def test_url_validation_accepts_valid_formats(self, valid_url):
        ...
        assert is_valid, f"Expected valid URL to be accepted, but was rejected: {repr(valid_url)}"
        ...
        assert is_valid_ks, (
            f"Expected valid URL to be accepted by knowledge source validator, but was rejected: {repr(valid_url)}"
        )

        # Should not have an error message
        assert error_message is None, f"Expected no error message for valid URL, but got: {error_message}"
    ...
    def test_url_validation_is_deterministic(self, url):
        ...
        assert result1 == result2 == result3, f"URL validation is not deterministic for: {repr(url)}"

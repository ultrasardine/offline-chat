    ...
    **kwargs
    ...
    if connection_name:
        ...
        if not username or password is None:
            raise ValueError(
                "Oracle TNS connection requires 'username' and 'password'"
            )
        return MCPServerConfig(
            name=name,
            command="sql",
        ...
        if not all([host, service_name, username]) or password is None:
            raise ValueError(
                "Oracle full connection requires 'host', 'service_name', "
...
def _create_postgresql_config(
    ...
    **kwargs
    ...
    if not all([host, database, username]) or password is None:
        raise ValueError(
            "PostgreSQL configuration requires 'host', 'database', "
            "'username', and 'password'"
        )
...
def _create_mysql_config(
    ...
    **kwargs
    ...
    if not all([host, database, username]) or password is None:
        raise ValueError(
            "MySQL configuration requires 'host', 'database', "
            "'username', and 'password'"
        )

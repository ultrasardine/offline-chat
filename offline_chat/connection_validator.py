...
async def _validate_connection_async(config: MCPServerConfig) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        logger.info(f"Executing test query '{test_query}' using tool '{query_tool_name}'...")
        result = await client.call_tool(query_tool_name, {"query": test_query})
